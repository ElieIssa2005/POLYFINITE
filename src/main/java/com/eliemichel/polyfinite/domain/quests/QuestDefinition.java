...
import com.eliemichel.polyfinite.domain.progression.RewardType;
import com.eliemichel.polyfinite.domain.quests.types.*;
    ...
    public Quest createQuest(int levelNumber, int questIndex) {
        ...
        switch (type) {
            case DESTROY_ENEMIES:
                return new DestroyEnemiesQuest(
                    questId, targetValue, reward);

            case DESTROY_ENEMY_TYPE:
                return new DestroyEnemyTypeQuest(
                    questId, targetSpecifier, targetValue, reward);

            case DESTROY_WITH_TOWER:
                return new DestroyWithTowerQuest(
                    questId, targetSpecifier, targetValue, reward);

            case SPEND_COINS_TOTAL:
                return new SpendCoinsTotalQuest(
                    questId, targetValue, reward);

            case SPEND_COINS_ON_TOWER:
                return new SpendCoinsOnTowerQuest(
                    questId, targetSpecifier, targetValue, reward);

            case GAIN_SCORE:
                return new GainScoreQuest(
                    questId, targetValue, reward);

            case BUILD_TOWERS:
                return new BuildTowersQuest(
                    questId, targetSpecifier, targetValue, reward);

            case UPGRADE_ANY_TO_LEVEL:
                return new UpgradeAnyToLevelQuest(
                    questId, targetValue, reward);

            case UPGRADE_TOWER_TO_LEVEL:
                return new UpgradeTowerToLevelQuest(
                    questId, targetSpecifier, targetValue, reward);
